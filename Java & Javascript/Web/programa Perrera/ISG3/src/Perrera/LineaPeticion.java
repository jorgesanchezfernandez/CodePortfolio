		...
		if(peticiones.size()<=MAX_PETICIONES){
			for (Iterator iter = peticiones.iterator(); iter.hasNext();) {
				...
				if (pet.getAnimal().getNumSerie().equals(ani.getNumSerie())) {
					System.out.println("Este animal ya ha sido añadido a las peticiones.");
					encontrado = true;
				}
			}				
			if(!encontrado){
			...
			throw new IndexOutOfBoundsException("No puede añadirse mas peticiones, maximo alcanzado");
